  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#f8faff] text-slate-900 font-sans">
      <Sidebar 
        ...
        onNavigate={() => setIsMobileMenuOpen(false)}
      />
      {isMobileMenuOpen && <button aria-label="Tutup menu" onClick={() => setIsMobileMenuOpen(false)} className="fixed inset-0 z-40 bg-slate-950/30 lg:hidden" />}
      <div className="flex-1 flex flex-col min-w-0 h-full relative">
        <Topbar onMenuClick={() => setIsMobileMenuOpen(true)} />
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 lg:p-10 custom-scrollbar">
          {children}
        </main>
      </div>
    </div>
