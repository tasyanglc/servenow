...
export async function GET() {
  try {
    ...
    const [{ rows }, { rows: deals }, { rows: projects }] = await Promise.all([db.query('SELECT project_id, data FROM tasks'), db.query('SELECT value, expected_revenue, stage FROM deals'), db.query('SELECT id, name, status FROM projects')]);
    const tasks = rows.map(row => ({ ...row.data, projectId: row.project_id || row.data.projectId })); const total = Math.max(tasks.length, 1);
    ...
    const projectRows = projects.map(project => { const related = tasks.filter(task => task.projectId === project.id); const achievedTasks = related.filter(task => task.historical_actual_breached === false || task.status === 'Resolved').length; return { ...project, taskCount: related.length, progress: related.length ? Math.round(achievedTasks / related.length * 100) : project.status === 'Completed' ? 100 : project.status === 'At risk' ? 45 : 65, owner: related[0]?.owner?.name || 'Unassigned' }; });
    const statusCounts = { achieved, atRisk: tasks.filter(task => risk(task) === 'AT RISK').length, overdue, onTrack: tasks.filter(task => risk(task) === 'ON TRACK').length };
    return Response.json({ taskCount: tasks.length, slaAchievement: Math.round(achieved / Math.max(historical.length, 1) * 100), weeklyExceptions: atRisk, criticalExceptions: exceptions, deptSummary, trend, directorDependency: `${Math.round(atRisk / total * 100)}%`, pipelineValue: deals.reduce((sum, deal) => sum + Number(deal.value || 0), 0), weightedPipeline: deals.reduce((sum, deal) => sum + Number(deal.expected_revenue || 0), 0), dealStages: Object.entries(deals.reduce((acc, deal) => ({ ...acc, [deal.stage]: (acc[deal.stage] || 0) + Number(deal.value || 0) }), {})).map(([name, value]) => ({ name, value })), projectSummary: { total: projects.length, completed: projects.filter(project => project.status === 'Completed').length }, projectRows, statusCounts, updatedAt: new Date().toISOString() });
